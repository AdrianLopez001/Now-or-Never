...
export default function Dashboard() {
  ...
  const getBolao = () => {
    ...
    matchPreds.forEach(({ match, pred }) => {
      if      (pred.probHomeWin       > 0.80) selections.push({ match, desc: `Vitória do ${match.homeTeamName}`,        prob: pred.probHomeWin,       odd: toOdd(pred.probHomeWin) });
      else if (pred.probAwayWin       > 0.80) selections.push({ match, desc: `Vitória do ${match.awayTeamName}`,        prob: pred.probAwayWin,       odd: toOdd(pred.probAwayWin) });
      else if (pred.probDraw          > 0.80) selections.push({ match, desc: `Empate`,                                    prob: pred.probDraw,          odd: toOdd(pred.probDraw) });
      else if (pred.probDoubleChance1X > 0.80 && pred.probDoubleChance1X > pred.probDoubleChanceX2)
      ...
      else if (pred.probDoubleChanceX2 > 0.80) selections.push({ match, desc: `${match.awayTeamName} ou Empate (X2)`, prob: pred.probDoubleChanceX2, odd: toOdd(pred.probDoubleChanceX2) });
    });
      ...
      {bolao && (
        <div className="space-y-3">
          <h2 className="text-sm font-black text-gray-200 uppercase tracking-wider border-l-4 border-emerald-500 pl-3">
            🏆 Recomendação de Bolão Inteligente (IA) — Eventos &gt; 80%
          </h2>
              ...
              <div className="flex flex-col justify-between border-t md:border-t-0 md:border-l border-gray-800 pt-4 md:pt-0 md:pl-6">
                <div>
                  ...
                  <p className="text-xs text-gray-400 leading-relaxed italic">
                    Bilhete montado reunindo os eventos com probabilidade superior a 80% nos mercados de resultado final e chance dupla, maximizando a taxa de acerto estatístico.
                  </p>
                </div>
